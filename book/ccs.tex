...
%\setCJKmainfont{SourceHanSansSC-Medium} % "SC" 表示简体中文版本 思源黑体
\setCJKmainfont{SourceHanSerifSC-SemiBold} % "SC" 表示简体中文版本  思源宋体
...
{\vspace{ 0.2cm } \noindent \textbf { #1 } \vspace*{ 0.05cm } \nopagebreak[4]
 \addcontentsline{toc}{paragraph}{\hspace*{1.2cm}{#1}}
}

\NewDocumentCommand { \mySamllsubsection } { m }
{\vspace{ 0.2cm } \noindent \textbf { #1 } \vspace*{ 0.05cm } \nopagebreak[4]
 \addcontentsline{toc}{subparagraph}{\hspace*{2.0cm}{#1}}
}
...
\NewDocumentCommand { \myChapterNoContents } { mmm }
{
    \color{black}
    \pagecolor{white}
    %\ThisULCornerWallPaper{1.0}{images/section-header.png}
...
\NewDocumentCommand { \myPartGray } { mmm }
{
  \pagecolor{gray}
  \color{white}
  \section*{\zihao{2}{#1}\hspace{0.5cm}{#2}}
