...
另一方面，客户端代码试图更改访问器获取的数据是可疑的。如果修改了数据的本地副本，它将不再与对象的值匹配；因此，语义会发生变化。在某些情况下，这是有意为之，但要非常小心。最好复制或引用实例数据以保留对象的语义。代码始终可以在需要时访问实例变量值，并且始终与对象保持一致。本地保存数据允许修改和更改其含义。

\filename{清单9.33 使用常量防止突变并提高性能}
